...
export function axiosServerTemplate(_path: string) {
  const ts = typescript(path.resolve(_path, "src", "axios.server.ts"))
...
export function addInterceptor(instanceKey: string | 'global', interceptor: Interceptor) {
...
function applyInterceptor(instance: AxiosInstance, interceptor: Interceptor, key: string) {

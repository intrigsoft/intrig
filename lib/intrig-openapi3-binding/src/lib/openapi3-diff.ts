  ...
  return produce<Differences>({}, (draft) => {
    ...
    commonPaths.forEach((path) => {
      ...
      commonMethods.forEach((method) => {
        if (!_.isEqual(oldMethods[method], newMethods[method])) {
          const methodDiff: {
            method: string;
            parameters?: {
              added?: OpenAPIV3_1.ParameterObject[];
          ...
          // Compare parameters
          const oldParams: OpenAPIV3_1.ParameterObject[] = oldMethods[method].parameters || [];
          const newParams: OpenAPIV3_1.ParameterObject[] = newMethods[method].parameters || [];
          ...
          if (!_.isEqual(oldMethods[method].requestBody, newMethods[method].requestBody)) {
            methodDiff.requestBody = {
              old: oldMethods[method].requestBody as OpenAPIV3_1.RequestBodyObject,
              new: newMethods[method].requestBody as OpenAPIV3_1.RequestBodyObject
            };
          }

          // Compare responses
          const oldResponses = oldMethods[method].responses || {};
          const newResponses = newMethods[method].responses || {};
    ...
    commonComponents.forEach((component) => {
      ...
      commonFields.forEach((field) => {
        if (!_.isEqual(oldComponent[field], newComponent[field])) {
          modifiedFields.push({
            field,
            old: oldComponent[field],
            new: newComponent[field]
          });
        }
      });

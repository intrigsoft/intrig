...
export async function getOpenApiSpec(url: string, config: IntrigConfig): Promise<OpenAPIV3.Document> {
  try {
    const response = await fetchSpec(url, config.rejectUnauthorized ?? true)
    const parsedResponse = JSON.parse(response)
    return await RefParser.bundle(parsedResponse) as OpenAPIV3.Document
  } catch (error) {
